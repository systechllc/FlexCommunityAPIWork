...
namespace Flex.Smoothlake.FlexLib
{
    public class CWX : ObservableObject
    {
        ...
        private void QueueMessageReply(int seq, uint resp_val, string reply)
        {
            ...
            int char_pos, block;
            bool b = int.TryParse(words[0], out char_pos);
            ...
            OnMessageQueued(block, char_pos);
        }

        public int Send(string s)
        {
            return _radio.SendCommand("cwx send \"" + s + "\"");
        }

        public int Send(string s, int block)
        {
            return _radio.SendReplyCommand(new ReplyHandler(QueueMessageReply), "cwx send \"" + s + "\" " + block);
        }
        ...
        public delegate void MessageQueuedEventHandler(int sequence, int buffer_index);
        public event MessageQueuedEventHandler MessageQueued;

        private void OnMessageQueued(int sequence, int buffer_index)
        {
            if (MessageQueued != null)
                MessageQueued(sequence, buffer_index);
        }

        public delegate void CharSentEventHandler(int index);
        public event CharSentEventHandler CharSent;

        private void OnCharSent(int index)
        {
            if (CharSent != null)
                CharSent(index);
        }
